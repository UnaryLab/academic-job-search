...
- One table row per posting with 15 cells in header order: university, deadline (`YYYY-MM-DD`, `rolling`, or `unknown`), title, area (a key from `areas.md`, comma-joined if several), rank (`assistant|associate|full|open`, the lowest rank the ad accepts; `open` when all ranks), application link (`<a href>Apply</a>`), flag (`ok|unverified|deadline-unclear`), stated priority (the hiring areas the ad itself names, `none stated (all-areas)` otherwise), fit notes, department, country, required materials, references (count of letters or referee contacts the ad asks for: `3`, `3+`, `4`, or `unknown`; note "letters" vs "contacts" in materials), contact, date checked. `class="flagged"` on the `<tr>` when flag != ok; `class="wide"` on materials and notes cells. Rows sorted by deadline ascending, `rolling`/`unknown` last; flagged rows after all `ok` rows in the same order, with `unverified` rows last.
