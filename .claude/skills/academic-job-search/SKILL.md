...
description: Search open faculty positions in computer architecture, AI/ML hardware, and quantum error correction (areas, universities, and minimum rank configurable in areas.md, universities.md, and rank.md at the project root) at North America/EU/Asia top-150 universities; write a dated self-contained HTML report to output/. Use when the user asks to run the faculty job search, refresh job listings, or find new faculty openings.
---
...
Finds currently open tenure-track/tenured faculty positions, plus permanent or continuing teaching-stream faculty posts (area `teaching`), at North American, European, and Asian top-150 universities in the areas listed in `areas.md`. Each run starts from the entries in the last report (`entries.json`), verifies those not yet confirmed twice, adds newly found postings, and drops an entry only when a verifier rejects it or its deadline has passed. One run = one dated HTML file. Not for postdoc, sessional or definite-term lecturer, staff, or industry positions.
...
- `universities.md` (project root): the static North America/EU/Asia top-150 union list with an `Aliases` column (short forms separated by `;`). Rebuild only when the user asks; add an alias when a real posting names a listed university by a form not yet in the table.
- `merge.py` (this skill directory): matches, dedupes, sorts, and renders the agent JSON files.
- `entries.json` (this skill directory): the full entry list of the last rendered report; `merge.py` rewrites it after each render (`--no-save` skips). Each entry carries `verified`, the number of runs in which a verifier loaded the ad and kept the entry (0 when absent).
- `areas.md` (project root): the research areas, with area key, name, search terms, adjacent fields that count, the units to check (departments, schools, or colleges whose hiring pages the sweep opens, matched by meaning rather than exact name), and excluded units.
- `rank.md` (project root): the minimum rank the user applies at (`Minimum rank` row, default `assistant`; `merge.py --min-rank` overrides) and the title-to-rank mapping. An ad counts only when it accepts applicants at that rank or below; `open` rank always counts.
- `template.html` (this skill directory): self-contained report template with `<!--DATE-->`, `<!--COUNT-->`, `<!--AREAS-->`, `<!--SCOPE-->`, `<!--ROWS-->`, `<!--GAPS-->` markers (`<!--AREAS-->` is the area names from `areas.md`; `<!--SCOPE-->` is the Region values and the top-N from the heading of `universities.md`; `<!--OPTIONS-->` is a JSON object `{areas: [{key, name}], regions: [...], universities: [{name, country, region}]}` from `areas.md` and `universities.md` that the page's area, region, and university dropdowns are built from).
- Run date (today).
...
- One table row per posting with 15 cells in header order: university, deadline (`YYYY-MM-DD`, `rolling`, or `unknown`), title, area (a key from `areas.md`, comma-joined if several), rank (`assistant|associate|full|open`, the lowest rank the ad accepts; `open` when all ranks), application link (`<a href>Apply</a>`), flag (`ok|unverified|deadline-unclear`), stated priority (the hiring areas the ad itself names, `none stated (all-areas)` otherwise), fit notes, department, country, required materials (each document with its stated length in parentheses, e.g. `research statement (3-4 pp)`, `cover letter (500 words)`; `no lengths stated` at the end when the ad gives none), references (count of letters or referee contacts the ad asks for: `3`, `3+`, `4`, or `unknown`; note "letters" vs "contacts" in materials), contact, date checked. `class="flagged"` on the `<tr>` when flag != ok; `class="wide"` on materials and notes cells. Rows sorted by deadline ascending, `rolling`/`unknown` last; flagged rows after all `ok` rows in the same order, with `unverified` rows last.
...
1. Dispatch parallel search agents (general-purpose, one message, concurrent), each returning a JSON array of entries with the fields in the output contract:
   ...
   - Department-sweep agents covering every university in `universities.md`: split the table in order into lists of about 30 (one agent per list; North America rows first, then EU, then Asia; a scoped run splits only the rows in scope). Each agent works its list sequentially and searches `<university> faculty opening <search terms from areas.md, slash-joined>` plus the hiring pages of the units named in the `Units to check` column of `areas.md` (the union over all areas, found under whatever name the university uses) and the university's central faculty-jobs portal, and returns in its final message its gaps: each university it could not check, with what blocked and the URL to check.
2. Merge: copy `entries.json` to `<dir>/00-previous.json` so the previous report's entries are merged as a source. Write the agent JSON files to the same directory and run `conda run -n base python merge.py --date <date> --in-dir <dir>`. It drops entries whose rank is above `--min-rank` (default from `rank.md`: associate-only, full-only, and chair searches go). It keeps an entry only when its `university` field, normalized (lowercase, punctuation stripped, spaces collapsed), exactly equals a canonical name or alias in `universities.md`; nothing fuzzy. Entries whose title or link names a branch campus (`hkust-gz`, `hkust(gz)`, `cuhk-shenzhen`, `cuhk shenzhen`, `nyu abu dhabi`, `nyu shanghai`, `duke kunshan`) are dropped. Every drop is printed with its reason. Within a university, two entries are the same posting when their link keys match (Interfolio id, or same host and same 5+ digit id) or their title token-set Jaccard is at least 0.5; same host with different numeric ids means distinct postings. The kept entry is the higher-scoring one (+3 university-domain or Interfolio link, -2 board mirror, +2 dated deadline, +1 rolling, +2 flag `ok`, +1 `deadline-unclear`); it takes the firmest deadline of the pair, the higher `verified` count, and notes the other link as `[also listed: url]`.
3. Verify the merged list before rendering. Every entry from `00-previous.json` with `verified` below 2 is verified, unless its `checked` date equals the run date (a same-day rerun does not re-verify); entries with `verified` of 2 or more are skipped (already confirmed in two runs; `merge.py` still drops them once their deadline passes). Also select every new entry that meets any of these: flag is `unverified`; deadline is rolling or unknown and no posted date is recorded; the link is a search, category, listing, or board-mirror page rather than the ad itself; the deadline or title came from a search snippet the agent did not open; the notes say the areas were copied from a previous round. Dispatch at most 6 general-purpose verifiers, grouping the selected entries evenly (parallel, one message). Each verifier loads the ad itself (or the university's portal), records in `materials` the stated length of every document (or `no lengths stated`), then returns for each entry one of: `confirmed` with corrected fields and the direct ad URL; `corrected` with what changed; `drop` with the reason (past cycle, not faculty, conflated with a postdoc or news item, does not exist). Apply the results to the files in `<dir>` (including `00-previous.json`): update fields, on `confirmed` or `corrected` with the ad loaded set `verified` to its previous value plus 1 (an `unverified` outcome leaves the count as is), keep `unverified` only where the verifier could not load anything, and remove entries the verifier rejected; an existing entry is removed only on a `drop` verdict. Write the outcome to `verify.json` in the scratchpad, one record per entry: link key, action (`confirmed`, `corrected`, `drop`), what changed. The drops go in the user report with reasons.
...
- Rank: the ad must accept applicants at the minimum rank in `rank.md` (default assistant); search and verify agents skip senior-only posts (associate/full/chair only, tenured-only professorships) instead of returning them, and set the `rank` field by the title mapping in `rank.md` (the lowest rank the ad accepts).
- Tenure-track/tenured research faculty, or permanent/continuing teaching-stream faculty in the units `areas.md` lists for `teaching` (tagged `teaching`; teaching-stream Lecturer and Assistant Professor of Teaching map to rank `assistant`); ads in the adjacent fields listed in `areas.md` count when they plausibly cover one of the areas. An ad from a unit named in an area's `Excluded units` column never counts for that area, whatever the ad text says.
...
- Materials: list every document the ad asks for, each with its stated page or word limit in parentheses; write `no lengths stated` when the ad gives none. Never guess a length.
...
- `universities.md`, `areas.md`, `rank.md` at the project root (user inputs); `template.html`, `merge.py`, `entries.json` in this skill directory.
- Boards: academicjobsonline.org, cra.org/ads, higheredjobs.com, jobs.ac.uk, academicpositions.com, jobs.nature.com.

## Failure modes

- **Web-search budget**: WebSearch is capped per session (200 calls by default) and the cap is shared by every subagent in the session, so a second run in the same session, or 13 agents each searching freely, runs dry and falls back to direct page fetches. Run the search in a fresh session, or raise `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`; tell agents to cap searches at about 3 per university and prefer department pages.
...
- **JS-only application pages**: Interfolio ads (`apply.interfolio.com/<id>`) render nothing in WebFetch; read the public JSON at `https://logic.interfolio.com/dossier-api/positions/<id>` (fields `start_date`, `end_date`, description; Asia-hosted ads at `apply.ap1.interfolio.com` use `logic.ap1.interfolio.com`). For bot-walled boards (HigherEdJobs, CRA) prefix the URL with `https://r.jina.ai/`.
- **AcademicJobsOnline listing URL**: the `ajo?joblist-...` form returns HTTP 410; use `https://academicjobsonline.org/ajo?joblst-0-0-0-0------` (full list) or the field boards `/ajo/cs`, `/ajo/EE`, `/ajo/QC`.
